        ...
        for i, (x, mask, token_type, y1_sec, y2_sec, arg1_mask, arg2_mask) in enumerate(train_loader):
            ...
            if total_batch % args.evaluate_steps == 0:
                ...
                if (acc_sec + f1_sec) > (dev_best_acc_sec + dev_best_f1_sec):
                    ...
                    torch.save(model.state_dict(), args.save_folder + 'best_model.ckpt')
                    improve = '*'
                ...
                lgg.info(
                    'SEC: Iter: {0:>6},  Train Loss: {1:>5.2},  Train Acc: {2:>6.2%}, Val Loss: {3:>5.2},  Val Acc: {4:>6.2%}, Val F1: {5:>6.2%} Time: {6} {7}'.format(
                        total_batch, loss.item(), train_acc_sec, loss_dev, acc_sec, f1_sec, time_dif, improve))
...
def test(args, model, test_loader):
    model.load_state_dict(torch.load(args.save_folder + 'best_model.ckpt'))
    model.eval()
    # start_time = time.time()
    ...
    # lgg.info(report_sec)
    #
    # np.savetxt(args.save_folder + 'preds.csv', predictions, fmt='%d', delimiter=',')

    print("According to CoNLL method:")
    expand_eval(args, model, test_loader, ckpt_path=None)
    acc_sec_conll, f1_sec_conll = CoNLL_eval(args,model,test_loader,args.save_folder + 'best_model.ckpt')
    print("Test Acc: {:.2%}, Test F1: {:.2%}".format(acc_sec_conll, f1_sec_conll))

    print("\nAccording to Dup method:")
    acc_sec_dup, f1_sec_dup = dup_eval(args,model,test_loader,args.save_folder + 'best_model.ckpt')
    print("Test Acc: {:.2%}, Test F1: {:.2%}".format(acc_sec_dup, f1_sec_dup))

    print("\nAccording to Expand method:")
    acc_sec_expand, f1_sec_expand = expand_eval(args,model,test_loader,args.save_folder + 'best_model.ckpt')
    print("Test Acc: {:.2%}, Test F1: {:.2%}".format(acc_sec_expand, f1_sec_expand))

    test_acc_dict = {"GOLF":{},"dup":{},"expand":{},"conll":{}}

    test_acc_dict["GOLF"]["acc"]=acc_sec
    test_acc_dict["GOLF"]["f1"] = f1_sec

    test_acc_dict["dup"]["acc"] = acc_sec_dup
    test_acc_dict["dup"]["f1"] = f1_sec_dup

    test_acc_dict["expand"]["acc"] = acc_sec_expand
    test_acc_dict["expand"]["f1"] = f1_sec_expand

    test_acc_dict["conll"]["acc"] = acc_sec_conll
    test_acc_dict["conll"]["f1"] = f1_sec_conll

    with open('{}/test_results.json'.format(args.save_folder), 'w') as fp:
        json.dump(test_acc_dict, fp,indent=4)
